  ...
  return (
    ...
    <div className="row justify-content-center align-content-md-stretch" dir="rtl">
      {items.map((item, index) => (
        <a key={index} href={`#${index}`} onClick={() => handleClick(item.title)} className="m-2  col-lg-4 col-xl-5 col-sm-12 col-xs-12 col-md-6 shadow-3 border-col rounded-4 link">
          <div className=" p-2 text-Light">
            <span className=' d-block'>{item.title}</span>
            <small className="fw-light  text-muted font-small  ">{item.description}</small>
          </div>
        </a>
      ))}
    </div>
